...
-- VIEW 
-- VIEW_EMP01 VIEW(가상테이블) 만든다. 실제테이블(사원), 가상주체(사원아이디, 사원이름, 부서아이디)
CREATE VIEW VIEW_EMP01 AS SELECT EMPLOYEE_ID, FIRST_NAME, DEPARTMENT_ID
FROM EMPLOYEES WHERE DEPARTMENT_ID = 10 ORDER BY EMPLOYEE_ID ASC;
SELECT * FROM VIEW_EMP01;

CREATE TABLE TBL_EMP01 AS SELECT EMPLOYEE_ID, FIRST_NAME, DEPARTMENT_ID
FROM EMPLOYEES WHERE DEPARTMENT_ID = 10 ORDER BY EMPLOYEE_ID ASC;
SELECT * FROM TBL_EMP01;